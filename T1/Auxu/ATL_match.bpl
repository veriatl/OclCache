...
procedure CS2RS_matchAll() returns ();
  requires (forall cs: ref :: cs!=null && read($srcHeap, cs, alloc) && dtype(cs) <: HSM$CompositeState ==>
 false  ==>
getTarsBySrcs(Seq#Singleton(cs)) ==null || !read($tarHeap, getTarsBySrcs(Seq#Singleton(cs)), alloc)); 
  modifies $tarHeap,$linkHeap;
// Rule Outcome
  ensures (forall cs: ref ::
cs!=null && read($srcHeap, cs, alloc) && dtype(cs) <: HSM$CompositeState ==>
 false  ==>
getTarsBySrcs(Seq#Singleton(cs)) !=null 
&& read($tarHeap, getTarsBySrcs(Seq#Singleton(cs)), alloc)
&& dtype(getTarsBySrcs(Seq#Singleton(cs))) <: FSM$RegularState);
// Frame Condition
  ensures (forall<alpha> $o : ref, $f: Field alpha ::
	($o == null 
	|| read($tarHeap, $o, $f) == read(old($tarHeap), $o, $f) 
	|| ((dtype($o) <: FSM$RegularState) && Seq#Length(getTarsBySrcs_inverse($o)) == 1 && dtype(Seq#Index(getTarsBySrcs_inverse($o), 0)) <: HSM$CompositeState && $f==alloc )));
  free ensures $HeapSucc(old($tarHeap), $tarHeap);
  free ensures $HeapSucc(old($linkHeap), $linkHeap);
  free ensures surj_tar_model($srcHeap, $tarHeap);

function printGuard_CS2RS(hp: HeapType, cs: ref): bool
{  false  }
