...
(exists cs: ref :: 
cs!=null && read($s, cs, alloc) && dtype(cs) <: HSM$CompositeState 
 &&  false  &&
  getTarsBySrcs(Seq#Singleton(cs)) == rs2)
...
t1!=null && read($s, t1, alloc) && dtype(t1) <: HSM$Transition 
 ...
 &&  read($s, t1, HSM$Transition.source) == src && read($s, t1, HSM$Transition.target) == trg && read($s, c, HSM$AbstractState.compositeState) == src && !(dtype(trg) == HSM$CompositeState)  &&
